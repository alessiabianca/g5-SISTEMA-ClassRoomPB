...
public class MatriculaFilaCronologiaTest {
    ...
    public void deveManterOrdemCronologicaEstritaFilaEsperaFIFO() throws Exception {
        ...
        matriculaService.solicitarMatricula("ALUNO_B", "ES30", "2027.1"); // Deve ser o 1º da fila (Índice 1 no arquivo)
        matriculaService.solicitarMatricula("ALUNO_C", "ES30", "2027.1"); // Deve ser o 2º da fila (Índice 2 no arquivo)
        matriculaService.solicitarMatricula("ALUNO_D", "ES30", "2027.1"); // Deve ser o 3º da fila (Índice 3 no arquivo)
        ...
        assertEquals("O arquivo plano deve conter exatamente 4 registros persistidos", 4, salvas.size());
        ...
        assertEquals("O primeiro elemento da fila (índice 1) deve ser o ALUNO_B", "ALUNO_B", salvas.get(1).getMatriculaAluno());
        assertEquals("O status do ALUNO_B deve ser ESPERA", Matricula.StatusMatricula.ESPERA, salvas.get(1).getStatus());

        assertEquals("O segundo elemento da fila (índice 2) deve ser o ALUNO_C", "ALUNO_C", salvas.get(2).getMatriculaAluno());
        assertEquals("O status do ALUNO_C deve ser ESPERA", Matricula.StatusMatricula.ESPERA, salvas.get(2).getStatus());

        assertEquals("O terceiro elemento da fila (índice 3) deve ser o ALUNO_D", "ALUNO_D", salvas.get(3).getMatriculaAluno());
        assertEquals("O status do ALUNO_D deve ser ESPERA", Matricula.StatusMatricula.ESPERA, salvas.get(3).getStatus());
    }
}
