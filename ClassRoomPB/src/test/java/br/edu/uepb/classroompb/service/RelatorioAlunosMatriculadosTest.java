...
public class RelatorioAlunosMatriculadosTest {
  ...
  public void deveGerarRelatorioComApenasAlunosConfirmadosDaTurma() throws Exception {
    ...
    matriculaRepository.salvar(
        new Matricula("ALUNO_002", "ES40", "2026.2", Matricula.StatusMatricula.CONFIRMADA));
    matriculaRepository.salvar(
        new Matricula("ALUNO_ESPERA", "ES40", "2026.2", Matricula.StatusMatricula.ESPERA));
    matriculaRepository.salvar(
        new Matricula("ALUNO_REJEITADO", "ES40", "2026.2", Matricula.StatusMatricula.REJEITADA));
    matriculaRepository.salvar(
        new Matricula("ALUNO_OUTRA_TURMA", "BD40", "2026.2", Matricula.StatusMatricula.CONFIRMADA));
    matriculaRepository.salvar(
        new Matricula(
            "ALUNO_OUTRO_PERIODO", "ES40", "2027.1", Matricula.StatusMatricula.CONFIRMADA));

    List<Matricula> relatorio = turmaService.gerarRelatorioAlunosMatriculados("ES40", "2026.2");

    assertNotNull(relatorio);
    assertEquals(
        "O relatório deve conter apenas as matrículas confirmadas da turma solicitada",
        2,
        relatorio.size());
