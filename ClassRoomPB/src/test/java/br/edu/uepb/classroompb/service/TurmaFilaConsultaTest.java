...
public class TurmaFilaConsultaTest {
  ...
  public void deveRetornarListaVaziaSemNullPointerExceptionSeNaoHouverFila() throws Exception {
    ...
    // Executa a busca para uma turma existente mas que não possui nenhuma matrícula com status
    // ESPERA
  ...
  public void deveRetornarContagemExataDeAlunosNaListaDeEspera() throws Exception {
    ...
    matriculaRepository.salvar(
        new Matricula(
            "0001", "ES35", "2027.1", Matricula.StatusMatricula.CONFIRMADA)); // Vaga física
    matriculaRepository.salvar(
        new Matricula("0002", "ES35", "2027.1", Matricula.StatusMatricula.ESPERA)); // 1º da Fila
    matriculaRepository.salvar(
        new Matricula("0003", "ES35", "2027.1", Matricula.StatusMatricula.ESPERA)); // 2º da Fila
