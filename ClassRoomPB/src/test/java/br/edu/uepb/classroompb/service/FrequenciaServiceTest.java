...
public class FrequenciaServiceTest {
    ...
    public void deveRegistrarChamadaEmLoteComSucessoQuandoProfessorForDonoDaTurma() throws Exception {
        ...
        Matricula m1 = new Matricula("2026101", disciplina, periodo, Matricula.StatusMatricula.CONFIRMADA); // Irá como PRESENÇA
        Matricula m2 = new Matricula("2026102", disciplina, periodo, Matricula.StatusMatricula.SOLICITADA);  // Irá como FALTA
        
        alunosAvaliados.add(m1);
        alunosAvaliados.add(m2);

        // EXECUÇÃO: Tenta consolidar a chamada no diário
        frequenciaService.registrarChamadaLote(profDono, disciplina, periodo, dataAula, alunosAvaliados);
    ...
    /**
     * BARREIRA DE SEGURANÇA: Impede que um professor tente efetuar ou alterar chamadas
     * em turmas de terceiros.
     */
    @Test
    public void deveLancarExcecaoEAbortarQuandoProfessorNaoForResponsavelPelaTurma() throws Exception {
        ...
        // EXECUÇÃO E ASSERÇÃO: O motor deve barrar e disparar ValidacaoException por quebra de segurança
        assertThrows(ValidacaoException.class, () -> {
            frequenciaService.registrarChamadaLote(profInvasor, disciplina, periodo, "27/06/2026", alunos);
        });
    ...
    public void deveLancarExcecaoSeAListaDeAlunosForEnviadaVazia() throws Exception {
        ...
        assertThrows(ValidacaoException.class, () -> {
            frequenciaService.registrarChamadaLote(prof, disciplina, periodo, "27/06/2026", new ArrayList<>());
        });
    }
}