...
/**
 * US34 — Serviço responsável pela apuração automatizada da situação acadêmica do aluno.
 * Centraliza a regra de negócio que cruza notas e frequência para determinar o status final.
 */
public class SituacaoAcademicaService {
    private final NotaRepository notaRepository;
    private final FrequenciaService frequenciaService;

    public SituacaoAcademicaService(NotaRepository notaRepository, FrequenciaService frequenciaService) {
        this.notaRepository = notaRepository;
        this.frequenciaService = frequenciaService;
    }

    /**
     * Calcula a média aritmética das notas de uma avaliação.
     * Se a nota3 for negativa (-1), considera apenas nota1 e nota2.
     */
     ...
     *
     * Regras (US34):
     *   1. Frequência < 75%                         → REPROVADO_FALTA (prioridade máxima)
     *   2. Frequência ≥ 75% e Média ≥ 7.0           → APROVADO
     *   3. Frequência ≥ 75% e Média ≥ 4.0 e < 7.0   → RECUPERAÇÃO
     *   4. Frequência ≥ 75% e Média < 4.0            → REPROVADO_NOTA
     */
     ...
     */
    public ResultadoApuracao apurarSituacao(String matriculaAluno, String codigoDisciplina, String periodo) 
            throws ValidacaoException {

        // 1. Busca a nota do aluno
        Nota nota = notaRepository.buscarPorAlunoEDisciplina(matriculaAluno, codigoDisciplina, periodo);
        if (nota == null) {
            throw new ValidacaoException("Erro: Nenhum registro de notas encontrado para o aluno '" 
                + matriculaAluno + "' na disciplina '" + codigoDisciplina + "' no período '" + periodo + "'.");
        }
        ...
        DesempenhoFrequencia desempenho = frequenciaService.calcularPercentualFrequencia(matriculaAluno, codigoDisciplina, periodo);
    ...
    /**
     * DTO interno que encapsula todos os dados consolidados de uma apuração acadêmica.
     * Facilita a exibição formatada no CLI sem expor detalhes internos do service.
     */
    public static class ResultadoApuracao {
        ...
        public ResultadoApuracao(Nota nota, double media, DesempenhoFrequencia desempenho, StatusAcademico status) {
            this.nota = nota;
            this.media = media;
        ...
        public DesempenhoFrequencia getDesempenho() { return desempenho; }
        public StatusAcademico getStatus() { return status; }
    }
}
