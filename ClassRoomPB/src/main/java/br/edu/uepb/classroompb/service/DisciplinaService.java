...
public class DisciplinaService {
    ...
    public void cadastrarDisciplina(String codigo, String nome, int cargaHoraria, int creditos, 
                                     List<String> preRequisitosInputs, String papelUsuarioLogado) 
        ...
        if (papelUsuarioLogado == null || !papelUsuarioLogado.equalsIgnoreCase("COORDENADOR")) {
            throw new ValidacaoException("Acesso negado: Apenas coordenadores podem cadastrar disciplinas.");
        }
        ...
        if (disciplinaExistente != null) {
            throw new ValidacaoException("Erro: Já existe uma disciplina cadastrada com o código '" + codigo + "'.");
        }
        ...
        if (preRequisitosInputs != null) {
            for (String prCodigo : preRequisitosInputs) {
                ...
                if (prExistente == null) {
                    throw new ValidacaoException("Erro: A disciplina de pré-requisito '" + prLimpo + "' não está cadastrada.");
                }
                
                if (prLimpo.equalsIgnoreCase(codigo.trim())) {
                    throw new ValidacaoException("Erro: Uma disciplina não pode ter a si mesma como pré-requisito.");
                }
        ...
        Disciplina novaDisciplina = new Disciplina(codigo.trim(), nome.trim(), cargaHoraria, creditos, preRequisitosValidados);
