...
public class NotaService {
    ...
    /**
     * [TASK 2523] Lógica de Busca de Notas no Serviço.
     * Varre e filtra as notas registradas no sistema associadas exclusivamente ao aluno e ao período ativo.
     * Caso o aluno esteja matriculado em uma disciplina que ainda não teve notas lançadas,
     * retorna uma estrutura padrão (zerada) para compor o painel visual sem falhas de carregamento.
     */
    public List<Nota> buscarNotasPorAlunoEPeriodo(String matriculaAluno, String periodo) throws ValidacaoException {
        ...
        List<Matricula> todasMatriculas = matriculaRepository.buscarTodas(); // Recuperação de matrículas[cite: 14]
        List<Nota> todasNotas = notaRepository.buscarTodas(); // Recuperação das notas persistidas[cite: 13]
        ...
        for (Matricula m : todasMatriculas) {
            if (m.getMatriculaAluno().equalsIgnoreCase(matriculaAluno) && 
                ...
                // 3. Se não houver nota lançada ainda, cria uma nota com valor 0.0 para visualização estável
                if (notaEncontrada != null) {
                    notasDoPeriodo.add(notaEncontrada);
                } else {
                    notasDoPeriodo.add(new Nota(matriculaAluno, m.getCodigoDisciplina(), periodo, 0.0, 0.0, -1.0));
                }
            }
        }
